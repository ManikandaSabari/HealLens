...
class AppointmentManager {
  ...
  async confirmBooking() {
    ...
    const isSaved = await this.saveAppointment(newAppointment);
    if (isSaved === false) {
      alert("Failed to save appointment to server. Please verify network and authentication.");
      return;
    }
  ...
  async saveAppointment(appt) {
    ...
    if (client && userId) {
      try {
        const dbPayload = {
          id: appt.id || appt.booking_reference,
          user_id: userId,
          ...
          patient_name: appt.patient_name || "Self",
          source: appt.source,
          created_at: appt.created_at
        };

        const { data, error } = await client.from("appointments").insert([dbPayload]);
        if (error) {
          console.error("[AppointmentManager] Supabase appointment insert failed:", error);
          return false;
        }
        return true;
      } catch (err) {
